...
def _apply_relationship_filter(participants, user, active_filter):
    if active_filter == FILTER_OWNERS_OF_FAVORITE_PROJECTS:
        favorite_ids = user.favorites.values_list("id", flat=True)
        return participants.filter(owned_projects__id__in=favorite_ids)
    if active_filter == FILTER_OWNERS_OF_PARTICIPATING_PROJECTS:
        participating_ids = user.participated_projects.values_list("id", flat=True)
        return participants.filter(owned_projects__id__in=participating_ids)
    if active_filter == FILTER_INTERESTED_IN_MY_PROJECTS:
        my_project_ids = user.owned_projects.values_list("id", flat=True)
        return participants.filter(favorites__id__in=my_project_ids)
    if active_filter == FILTER_PARTICIPANTS_OF_MY_PROJECTS:
        my_project_ids = user.owned_projects.values_list("id", flat=True)
        return participants.filter(participated_projects__id__in=my_project_ids)
    return participants
...
def users_list_view(request):
    ...
    if request.user.is_authenticated:
        ...
        participants = _apply_relationship_filter(participants, request.user, active_filter)
...
def skills_search_view(request):
    ...
    if q:
        qs = Skill.objects.filter(name__icontains=q).order_by("name")[:SKILLS_SEARCH_LIMIT]
    else:
...
def user_skill_add_view(request, user_id):
    ...
    if skill_id:
        ...
        return JsonResponse({"error": "no skill"}, status=HTTPStatus.BAD_REQUEST)
